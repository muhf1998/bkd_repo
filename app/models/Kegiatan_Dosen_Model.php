...
class Kegiatan_Dosen_Model
{
  ...
  {
    $query = "SELECT *, kegiatan_dosen.id as kegiatan_id 
                FROM kegiatan_dosen INNER JOIN users 
                  ON users.id = kegiatan_dosen.users_id";
  ...
  public function getByAkun($id)
  {
    $query = "SELECT *, kegiatan_dosen.id as kegiatan_id 
                FROM kegiatan_dosen INNER JOIN users 
                  ON users.id = kegiatan_dosen.users_id 
               WHERE kegiatan_dosen.users_id = '$id'";
  ...
  {
    $query = "INSERT INTO kegiatan_dosen (users_id,jenis_kegiatan,bukti_penugasan,sks,masa_penugasan,kinerja,bukti_dokumen,sks_2,rekomendasi,status,created_at) VALUE (:users_id, :jenis_kegiatan, :bukti_penugasan, :sks, :masa_penugasan, :kinerja, :bukti_dokumen, :sks_2, :rekomendasi, :status, :created_at)";
