      ...
      <div class="card-body">
        <form action="<?= BASEURL; ?>AuthController/ubah_profile_proses/<?= $data['user']['id'] ?>/<?= $data['role'] ?>" method="post" enctype="multipart/form-data">
          <div class="form-group mt-2">
            ...
            <input type="text" name="nama" id="nama" class="form-control" value="<?= $data['user']['nama'] ?>" required placeholder="...">
          </div>
          <div class="form-group mt-2">
            <label for="title">Gelar Depan</label>
            <input type="text" name="gelar_depan" id="gelar_depan" class="form-control" value="<?= $data['user']['gelar_depan'] ?>" required placeholder="...">
          </div>
          <div class="form-group mt-2">
            <label for="title">Gelar Belakang</label>
            <input type="text" name="gelar_belakang" id="gelar_belakang" class="form-control" value="<?= $data['user']['gelar_belakang'] ?>" required placeholder="...">
          </div>
          <div class="form-group mt-2">
          ...
          <div class="form-group mt-2">
            ...
            <select name="jabatan_fungsional" id="jabatan_fungsional" class="form-control">
              ...
              <?php foreach ($data['jabatan_fungsional'] as $jf) : ?>
                <option value="<?= $jf; ?>" <?= ($jf == $data['user']['jabatan_fungsional']) ? 'selected' : ''; ?>><?= $jf; ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group mt-2">
            <label for="title">Tempat Lahir</label>
            <input type="text" name="tempat_lahir" id="tempat_lahir" class="form-control" value="<?= $data['user']['tempat_lahir'] ?>" required placeholder="...">
          </div>
          <div class="form-group mt-2">
            <label for="title">Tanggal Lahir</label>
            <input type="text" name="tgl_lahir" id="tgl_lahir" class="form-control" value="<?= $data['user']['tgl_lahir'] ?>" required placeholder="...">
          </div>
